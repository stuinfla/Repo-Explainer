#!/usr/bin/env node
// build-kb-384.mjs — SINGLE 384-dim, STRUCTURE-AWARE RVF builder (recipe ADR-0001 v1.3.0).
//
...
//   - embedder = Xenova/bge-small-en-v1.5 (384-dim, CLS pooling, asymmetric query prefix)
...
//   <slug>-kb.rvf                384-dim HNSW store (ask-kb picks .rvf when no .big.rvf present)
//   <slug>-kb.rvf.embed.json     embedder config (model/pooling/queryPrefix) read by ask-kb
...
//
// Usage: node kb/build-kb-384.mjs --target photonlayer
...
function makeContext(target) {
  ...
  if (!fs.existsSync(repoDir)) throw new Error(`build-kb-384: repoDir not found: ${repoDir}`);
...
async function main() {
  ...
  console.log(`[build-kb-384] target=${slug} metaName=${target.metaName} | model=${MODEL} dim=${DIM}`);

  const ctx = makeContext(target);
  console.log(`[build-kb-384] repoDir=${ctx.repoDir}`);

  for (const rule of target.include || []) {
    const impl = RULE_IMPLS[rule.rule];
    if (!impl) { console.error(`[build-kb-384] unknown include rule "${rule.rule}" — skipped`); continue; }
    const n = impl(ctx, rule);
    console.log(`[build-kb-384] rule ${rule.rule.padEnd(18)} -> ${n} source(s)`);
  }
  ...
  if (!entries.length) { console.error('[build-kb-384] corpus is EMPTY'); process.exit(1); }
  ...
  console.log('[build-kb-384] @ruvector/rvf via:', rvfVia);
  ...
  console.log(`[build-kb-384] transformers via ${tVia} | model ${haveLocalModel ? 'local' : 'remote'} (${modelCache})`);
  ...
  console.log(`\n[build-kb-384] ingested ${ingested} vectors | passages lines ${passageLines}`);
  ...
  if (!ok) { console.error('[build-kb-384] RECONCILE FAILED'); process.exit(1); }
  console.log(`[build-kb-384] OK -> ${path.relative(__dirname, OUT_RVF)} (+embed.json,passages,ids) | size ${fs.statSync(OUT_RVF).size} bytes`);
}

main().catch((e) => { console.error('[build-kb-384] ERROR:', e); process.exit(1); });
